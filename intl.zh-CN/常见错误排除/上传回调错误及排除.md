...
-   InvalidArgument
    ...
    **说明：** 回调参数设置错误，或参数格式错误。常见的错误是`ArgumentValue`之间的回调参数，不是有效JSON格式。在 JSON中`\`、`"` 是转义字符，如`"callbackBody":"{"bucket":${bucket},"object":${object}}"`应该为`"callbackBody":"{\"bucket\":${bucket},\"object\":${object}}"`。针对具体的SDK，请参见对应的上传回调示例，详细请参考[SDK使用示例](intl.zh-CN/常见错误排除/上传回调错误及排除.md#section_gn4_xw3_wdb)部分。
