...
**说明：** 设置生命周期的API详细信息可参考[PutBucketLifecycle](../../../../intl.zh-CN/API 参考/关于Bucket的操作/PutBucketLifecycle.md#)。
...
-   策略：生命周期规则匹配的Object和碎片。
    ...
    -   按标签匹配：按指定标签的Key和Value匹配Object。单条规则可配置多个标签，OSS对所有拥有这些标签的对象执行生命周期规则。标签匹配不可以作用于碎片。
        ...
        **说明：** 对象标签功能正在公测中，您可以联系[售后技术支持](https://selfservice.console.aliyun.com/ticket/createIndex)申请体验此功能。详情请参见[对象标签](intl.zh-CN/开发指南/管理文件/对象标签.md#)。
...
|操作方式|说明|
|----|--|
|[控制台](../../../../intl.zh-CN/控制台用户指南/管理存储空间/设置生命周期.md#)|Web应用程序，直观易用|
|[Java SDK](../../../../intl.zh-CN/SDK 参考/Java/生命周期.md#)|丰富、完整的各类语言 SDK demo|
|[Python SDK](../../../../intl.zh-CN/SDK 参考/Python/生命周期.md#)|
|[PHP SDK](../../../../intl.zh-CN/SDK 参考/PHP/生命周期.md#)|
|[Go SDK](../../../../intl.zh-CN/SDK 参考/Go/生命周期.md#)|
|[C SDK](../../../../intl.zh-CN/SDK 参考/C/生命周期.md#)|
|[.NET SDK](../../../../intl.zh-CN/SDK 参考/C/生命周期.md#)|
|[Node.js SDK](../../../../intl.zh-CN/SDK 参考/Node.js/生命周期.md#)|
|[Ruby SDK](../../../../intl.zh-CN/SDK 参考/Ruby/生命周期.md#)|
...
在这个例子中，第一条规则会删除前缀为logs/，最后更新时间是10天前的Object。第二条规则虽然指定了删除2017年12月31日之前被修改的前缀为doc/的Object，但是由于它的Status是Disabled状态，所以该规则并不会生效。第三条规则会将标签为xx=1，最后更新时间是60天前的的Object存储类型修改为Archive（归档存储）。
...
-   通过设置生命周期转换文件类型或过期删除，是否有最小存储天数限制？
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4377/155557712434477_zh-CN.png)

    举例2：一个低频型文件，在创建10天后，通过CopyObject覆写，将存储类型转换为归档型时，会产生20天的低频型文件提前删除费用；文件的创建时间会更新，转换后的归档型文件，需再保存至少60天。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4377/155557712434481_zh-CN.png)
