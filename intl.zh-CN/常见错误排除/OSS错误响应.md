...
|IncorrectNumberOfFilesInPOSTRequest|Post请求中文件个数非法|400|Post请求表单域中只能有一个`file`域，排除请参见[PostObject错误及排查](intl.zh-CN/常见错误排除/Post Object错误及排查.md#)|
|InvalidArgument|参数格式错误|400|参数格式不符合要求，请对照相应[API](../../../../intl.zh-CN/API 参考/API概览.md#)|
|InvalidAccessKeyId|无效的AccessKeyId|403|AccessKeyId无效或过期，排除请参看[403错误及排查](intl.zh-CN/常见错误排除/OSS 403错误及排查.md#)|
|InvalidBucketName|无效的Bucket名字|400|Bucket命名规则请参看[开发人员指南](../../../../intl.zh-CN/开发指南/基本概念介绍.md#)|
|SymlinkTargetNotExist|符号链接指向的目标Object不存在|404|Object类型为符号链接，且符号链接指向的目标Object不存在|
|InvalidTargetType|符号链接指向的目标Object类型无效|400|Object类型为符号链接，且符号链接指向的目标Object类型仍为符号链接|
|InvalidObjectState|无效的Object状态|403|下载归档类型的Object时， -   没有提交RestoreObject请求或者上一次提交RestoreObject已经超时。
...
|InvalidEncryptionAlgorithmError|指定的熵编码加密算法错误|400|目前支持`AES256`和`KMS`两种加密方式，详见[PutObject](../../../../intl.zh-CN/API 参考/关于Object操作/PutObject.md#)|
...
|MissingContentLength|缺少内容长度|411|消息既非[chunked encoding](https://tools.ietf.org/html/rfc2616#section-3.6.1)又没有携带`Content-Length`|
|NoSuchBucket|Bucket不存在|404| |
|NoSuchKey|Object不存在|404| |
|NoSuchUpload|Multipart Upload ID不存在|404|没有初始化分片上传或者初始化的分片上传过期|
|NotImplemented|无法处理|400|OSS不支持的操作|
...
|DownloadTrafficRateLimitExceeded|下载流量超过限制|503| 默认上限是 5Gbit/s，包括内外网，有调整需求请[提交工单](https://workorder-intl.console.aliyun.com/console.htm#/ticket/createIndex)

 |
|UploadTrafficRateLimitExceeded|上传流量超过限制|503| 默认上限是 5Gbit/s，包括内外网，有调整需求请[提交工单](https://workorder-intl.console.aliyun.com/console.htm#/ticket/createIndex)
...
## OSS支持的操作中添加了不支持的参数 {#section_hrc_fs3_wdb .section}
