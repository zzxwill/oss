...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4406/15378848201520_zh-CN.png)
...
2.  应用服务器返回上传Policy和回调设置代码。
    ...
    上面消息body里面的callback内容是经过base64编码后生成的。经过base64解码后的内容如下：
...
-   设置成随机文件名
    ...
    如果想在上传时固定设置成随机文件名并且后缀保持跟客户端文件一致，可以将函数改为：
...
-   上传签名
    ...
    **说明：** 为了减少服务端的压力，设计思路是：初始化上传时，每上传一个文件后，获取一次签名。然后再上传时，比较当前时间与签名时间，看签名时间是否失效。如果失效，就重新获取一次签名，如果没有失效，就使用之前的签名。这里就用到了变量expire，核心代码如下：
    ...
    上面Policy中增加了starts-with，用来指定此次上传的文件名必须以user-dir开头，用户可自行指定此字符串。增加starts-with的原因是：在很多场景下，一个应用对应一个Bucket，为了防止数据覆盖，每个用户上传到OSS的文件都可以有特定的前缀。这样就存在一个问题，用户获取到这个Policy后，在失效期内都能修改上传前缀，从而上传到别人的目录下。为了解决这个问题，可以设置应用服务器在上传时就指定用户上传的文件必须是某个前缀。这样如果用户获取到了Policy也没有办法上传到别人的前缀上，从而保证了数据的安全性。
