...
如下图所示，您可以扫描二维码，安装示例程序。此工具是用Android开发的，本文档的应用服务器也适用于iOS。

![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/4399/6541_zh-CN.png)

示例应用的最终效果图如下：

**说明：** 应用服务器中的地址是示例地址。您可以本文提供的应用服务器代码示例，自己部署应用服务器。
...
本demo中，上传的方法支持普通上传和断点续传上传。在网络环境差的情况下，推荐使用断点续传上传。您还可以利用图片处理服务，对要上传的图片进行缩略和加水印处理。
...
-   应用服务器：即提供该Android/iOS应用的开发者开发的app后台服务，用于管理app上传和下载的Token，以及用户在app上传数据的元信息。

## 实现步骤 {#section_cyl_rsw_5db .section}

1.  移动应用向应用服务器申请一个临时上传凭证。

    **说明：** Android/iOS应用不能直接存储AccessKeyID/AccessKeySecret，这样会存在数据泄露的风险。所以应用必须向用户的应用服务器申请一个Token。这个Token是有时效性的，如果Token的过期时间是30分钟（由应用服务器指定），那么在这30分钟里面，该Android/iOS应用可以使用这个Token从OSS上传和下载数据， 30分钟后需要重新获取Token。
